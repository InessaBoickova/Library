...
export const Calendar = () => {
    ...
    const createData = (data) => {
        ...
        return {date,dayNumber,day,dayNumberInWeek,dayShort,year,month,monthShort,monthNumber,timestamp}
    }
                 ...
                 <div className='calendar__body-days' >
                    ...
                    // eslint-disable-next-line array-callback-return, consistent-return
                        createElem(createMonth().createMonthDays()).map((i) => {
                            ...
                            if (createData(selectBookingData).timestamp === i.timestamp){
                                return <button
                                        onClick={()=>onSubmit(new Date(i.timestamp))}
                            ...
                            if (i && (i.timestamp !== bookingDay.timestamp) && (i.dayNumberInWeek === - 1 || i.dayNumberInWeek === 5 )){
                            ...
                            if(i && (i.timestamp !== bookingDay.timestamp) 
                                ...
                                return (i.date.getTime() === dateNow.getTime()) 
                                        ? (createData(dateNow).dayNumberInWeek !== - 1 
                                                ...
                                                &&  <button onClick={()=> onSubmit(new Date(i.timestamp))}     
                            ...
                            if((i.timestamp === bookingDay.timestamp)){
                                return <button 
                                        onClick={()=>onSubmit(new Date(i.timestamp))} className='calendar__body-days-day calendar__body-days-active'
                                          type='button' > 
                                    {i.dayNumber} 
                                </button>
                            }
